...
torch::Tensor interp_forward(
    ...
    torch::Tensor radii,
    torch::Tensor fa_point,
    torch::Tensor fa_node,
    float beta,
    float inv_delta,
    int threads)
{
    ...
    CHECK_INPUT(radii);
    CHECK_INPUT(fa_point);
    CHECK_INPUT(fa_node);

    auto result = interp_cuda_forward(points, queries, centers, 
        children, pi_flat, pi_lengths, pi_starts, radii, fa_point, 
        fa_node, beta, inv_delta, threads);
    ...
    int threads)
{
    ...
    auto result = interp_cuda_backward(points, areas, queries, centers, children, 
        pi_flat, pi_lengths, pi_starts, ni_flat, ni_lengths, ni_starts, radii, 
        fan_point, fan_node, df_query, beta, inv_delta, threads);

    return result;
}


vector<torch::Tensor> initialize_features_fa(
    ...
    const int num_nodes)
{
    ...
    auto result = initialize_features_fa_cuda(features_point, areas, ni_flat, ni_lengths, ni_starts, num_nodes);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("initialize_features_fa", &initialize_features_fa, "initialize node and point features (area weighted)");
}
