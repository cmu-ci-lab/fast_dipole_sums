...
class PointSampler:
    ...
    def _find_first_zero_crossing(self, sdf):
        ...
        crossing = (next_sdf * prev_sdf < 0).long().argmax(1).long()
        return crossing
    ...
    def sample_intersection(self, rays_o, rays_d, near, far, sdf_func):
        with torch.no_grad():
        ...
        surf_interval_size = surf_upper - surf_lower
        surf_lower -= surf_interval_size * self.n_surf_samples / 2
        surf_upper += surf_interval_size * self.n_surf_samples / 2
        ...
        return z_vals, surf_z_image, near, far


    def sample_uniformly(self, rays_o, rays_d, near, far, override_n_samples=None):
        if override_n_samples is not None:
            z_vals = self.sample_interval_uniformly(override_n_samples, near, far)
        else:
            z_vals = self.sample_interval_uniformly(self.n_total_samples, near, far)
        return z_vals, None, near, far
