...
def eval_data(args):
    ...
    # default dtu values
    max_dist = 20 / args.scale
    visualize_threshold = 10 / args.scale
    thresh = 0.2 / args.scale  # downsample density
    ...
    stl_pcd = o3d.io.read_point_cloud(f'{args.dataset_dir}/{args.scan}/gt_pts_pruned.ply')
    ...
    over_all = (mean_d2s + mean_s2d) / 2 * args.scale
    ...
    data_color[ np.where(dist_d2s[:,0] >= max_dist)[0] ] = G
    ...
    stl_color[ np.where(dist_s2d[:, 0] >= max_dist)[0] ] = G
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mesh_name', type=str, default='new_bear.ply')
    ...
    print(eval_mesh(mesh_path, scan, exp_dir, 'public_data/bmvs', scale=300))
